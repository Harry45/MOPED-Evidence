...
Code: All functions related to the non-linear model.
"""
...
def quadratic(xvalues: torch.Tensor, parameters: torch.Tensor) -> torch.Tensor:
    """Calculates the quadratic function given the domain and the parameters.

    Args:
        xvalues (torch.Tensor): the domain of the function
        parameters (torch.Tensor): the parameters as input to the model
...
def generate_data_sin_quad(params: torch.Tensor, sigma: float, sin: bool) -> Tuple[torch.Tensor, torch.Tensor]:
...
def loglike_grid(data: torch.Tensor, xvalues: torch.Tensor, sigma: float) -> Tuple[dict, dict]:
...
def contour_plot(sin_dict: dict, quad_dict: dict, save: bool):
    """Makes a contour plot given two dictionaries with the following keywords:
        - amp_grid, ang_grid, logl, mle
        - a_grid, b_grid, logl, mle

    Args:
        sin_dict (dict): dictionary for the sinusoidal function
        quad_dict (dict): dictionary for the quadratic function
...
def derivatives(xvalues: torch.Tensor, parameters: torch.Tensor, sin: bool = True) -> torch.Tensor:
    """Calculates the derivatives of the two models (sinusoidal or quadratic)
    using PyTorch. One can do this analytically.

    Args:
        xvalues (torch.Tensor): the values of x (domain)
        parameters (torch.Tensor): the parameters input to the model
        sin (bool, optional): Derivatives of the sinusoidal function if True. Defaults to True.

    Returns:
        torch.Tensor: A matrix of size n x p, where n is the number of data and
    ...
    gradients = torch.as_tensor(gradients)
