...
Script: Involves non-linear models.
Date: February 2023
"""
...
# LOGGER.info('Now loading the non-linear models.')
...
class FirstModel:
    ...
    def function(parameters: torch.Tensor, domain: torch.Tensor) -> torch.Tensor:
        ...
        """
        target = parameters[0] * torch.sin(parameters[1] * domain)
        return target
    ...
    def gradient(parameters: torch.Tensor, domain: torch.Tensor) -> torch.Tensor:
        ...
        grad1 = torch.sin(parameters[1] * domain)
        grad2 = parameters[0] * domain * torch.cos(parameters[1] * domain)
